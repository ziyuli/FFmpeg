...
static int scroll_bilinear_slice(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    __m128i interp_h0_u16, interp_h1_u16, interp_v_u16;
    __m128i pixel0_u16, pixel1_u16, pixel2_u16, pixel3_u16;
    uint16_t pixel0_u16_arr[8], pixel1_u16_arr[8], pixel2_u16_arr[8], pixel3_u16_arr[8];
        ...
        for (int y = slice_start; y < slice_end; y++) {
            ...
            if (s->chroma || p == 0 || p == 3) {
                ...
                for (x = 0; x < w; x += 8) {               
                    ...
                    for (int i = 0; i < 8; i++) {
                        pixel0_u16_arr[i] = (uint16_t)(src[yy * in->linesize[p] + xx + i]);
                    }

                    for (int i = 0; i < 8; i++) {
                        xx_inc = FFMIN(xx + i + 1, w);
                        pixel1_u16_arr[i] = (uint16_t)(src[yy * in->linesize[p] + xx_inc]);
                    }

                    for (int i = 0; i < 8; i++) {
                        pixel2_u16_arr[i] = (uint16_t)(src[yy_inc * in->linesize[p] + xx + i]);
                    }

                    for (int i = 0; i < 8; i++) {
                        xx_inc = FFMIN(xx + i + 1, w);
                        pixel3_u16_arr[i] = (uint16_t)(src[yy_inc * in->linesize[p] + xx_inc]);
                    }

                    pixel0_u16 = _mm_load_si128((__m128i*)pixel0_u16_arr);
                    pixel1_u16 = _mm_load_si128((__m128i*)pixel1_u16_arr);
                    pixel2_u16 = _mm_load_si128((__m128i*)pixel2_u16_arr);
                    pixel3_u16 = _mm_load_si128((__m128i*)pixel3_u16_arr);

                    interp_h0_u16 = _mm_lerp_ep16(pixel0_u16, pixel1_u16, w_h_u16);
                    interp_h1_u16 = _mm_lerp_ep16(pixel2_u16, pixel3_u16, w_h_u16);
                    ...
                    // for (int i = 0; i < 8; ++i) {
                    //     dst[x + i] = (uint8_t)(interp_v_u16_ptr[i]);
                    // }
...
static const AVOption scroll_bi_options[] = {
    ...
    { "chroma",     "set full chroma interpolation",      OFFSET(chroma),  AV_OPT_TYPE_INT,   {.dbl=0.},   0, 1 , FLAGS},
    { NULL }
};
